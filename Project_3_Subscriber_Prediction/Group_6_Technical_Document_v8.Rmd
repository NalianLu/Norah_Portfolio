...
The 20% oversampling Decision Tree model performs best as of 0.1643227. We visualize and then fine-tune it.
```{r}
...
```
The performance for current decision tree is 0.1643227, which is equal to or larger than tuning results. Thus, we directly use the current decision tree with 20% oversampling p for model comparison in Chapter 4.
```{r}
tree_fine_tuned = rpart(adopter ~ ., data = balanced_data_train_20,
                        method = "class",
                        parms = list(split = "information"))
```
...
Decision Tree     | 0.1643227   | 20% oversampling
...
for (validation_rows in cv) {
  ...
  # Train model and evaluate its performance
  tree = rpart(adopter ~ ., data = balanced_data_train_cv,
               method = "class",
               parms = list(split = "information"))
  # make prediction
  pred_tree = predict(tree, data_validation_cv, type = "class")
  # get the performance
  conf_matrix = confusionMatrix(data = pred_tree,
...
Report the average and the standard deviation for performance
```{r}
cat("Decision Tree:","avg_performance is", mean(performance_cv_dt[!is.nan(performance_cv_dt)]),", standard deviation is", sd(performance_cv_dt[!is.nan(performance_cv_dt)]))
```
### 4.2.3 Naive Bayes
...
----------------- | ------------- | ---------------------
K-NN              | 0.0534887     | 0.00693262
Decision Tree     | 0.1392764     | 0.01720558
Naive Bayes       | 0.1745933     | 0.01710373

The Decision Tree model and Naive Bayes model have similar robustness, which are better than K-NN model.
