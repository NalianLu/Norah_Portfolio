...
```
There is no row with missing values in the data file.
...
We use ROSE (Random Over-Sampling Examples) package for oversampling. Remove user_id column because it is not a feature.
```{r}
...
```
We do oversampling on training data for 3 different scenarios.
```{r}
...
```
We define the normalize function and will use normalized data to calculate distance and train K-NN prediction models.
```{r}
# normalize
...
```
The 30% oversampling model performs best when k = 4 as of 0.063160. After k = 4, performance hovers around 0.060 and then comes below it.
...
```
The 20% oversampling model performs best when k = 4 as of 0.062685. Performance decreases gradually after k =4 and then goes back to 0.058.
...
```
The 10% oversampling model performs best when k = 3 as of 0.067485. Performance is below 0.06 after k = 4.

### 3.3.4 K-NN Feature Selection
The K-NN model with 10% oversampling and k = 3 has the best performance as of 0.067485. We then perform feature selection on it to select the best subset of features to avoid overfitting. We choose backward method because all features have a low correlation coefficient with adopter.
```{r}
...
```
The model performs best when "removed feature playlists" shows up with performance as of 0.09666835. The error occurs because after removing feature delta_shouts, all the distances are the same and the model cannot compare further.
...
# prepare training data with selected columns
balanced_data_train_norm_s <- balanced_data_train_norm[selected_features_knn]  # 14 columns
data_validation_norm_s <- data_validation_norm[selected_features_knn]
# train K-NN model with selected features 
knn_fine_tuned = knn(train = balanced_data_train_norm_s[,1:13],
                 test = data_validation_norm_s[,1:13],
                 cl = balanced_data_train_norm_s[,14],
                 k = 3)
# double-check model performance on validation data
# knn_cv(balanced_data_train_norm_s[,1:13],balanced_data_train_norm_s[,14],data_validation_norm_s[,1:13],3)
```
...
### 3.4.4 Decision Tree Feature Selection
The 20% oversampling Decision Tree model performs best as of 0.1643227. We then perform feature selection on it to select the best subset of features to avoid overfitting. We choose backward method because all features have a low correlation coefficient with adopter.
```{r}
# use data with 20% oversampling
X = balanced_data_train_20[,1:25]  # features
Y = balanced_data_train_20[,26]  # target variable
A = data_validation[,1:25]  # validation data features
B = data_validation[,26]  # validation data target variable
```
Define backward selection function.
```{r}
backward_selection_dt <- function(X,Y,A,B) {
  n_features <- ncol(X)
  selected_features <- seq_len(n_features)  # Initialize with all features
  ...
  for (i in seq_len(n_features)) {
    ...
    for (j in selected_features) {
      ...
      # validation data
      data_vali = cbind(A[current_features],B)
      # train a decision tree model
      tree = rpart(adopter ~ ., data = data_train,
                   method = "class",
                   parms = list(split = "information"))
      # make prediction
      pred_tree = predict(tree, data_vali, type = "class")
      # get the performance
      conf_matrix = confusionMatrix(data = pred_tree,
                              reference = data_vali$adopter,
                              mode = "prec_recall",
                              positive = '1')
      # evaluate performance
      precision <- conf_matrix$byClass["Pos Pred Value"]
      f1 <- conf_matrix$byClass["F1"]
      performance <-0.7*precision+0.3*f1
...
```{r, error=TRUE}
backward_selection_dt(X,Y,A,B)
```
Based on feature selection results, the Decision Tree model works at least the same best performance as of 0.1643227 when it only uses 'subscriber_friend_cnt'(column 7), 'lovedTracks'(column 9), 'songsListened', and 'avg_friend_age' 4 features.

### 3.4.5 Fine-Tune Decision Tree
We fine-tune the decision tree model based on feature selection and then visualize it.

```{r}
# selected features
selected_features_dt = c('subscriber_friend_cnt', 'lovedTracks', 'songsListened', 'avg_friend_age','adopter')
# prepare training data
data_train_dt = balanced_data_train_20[selected_features_dt]
# train Decision Tree model
tree_fine_tuned = rpart(adopter ~ ., data = data_train_dt,
                        method = "class",
                        parms = list(split = "information"))
# double-check model performance on validation data
#pred_tree = predict(tree_fine_tuned, #data_validation[selected_features_dt], type = "class")
#conf_matrix = confusionMatrix(data = pred_tree,
#                              reference = #data_validation$adopter,
#                              mode = "prec_recall",
#                              positive = '1')
#precision <- conf_matrix$byClass["Pos Pred Value"]
#f1 <- conf_matrix$byClass["F1"]
#performance_dt <- 0.7*precision+0.3*f1
#print(performance_dt)
```
...
prp(tree_fine_tuned, varlen = 0)
```
Besides feature selection, we also try different minimum information gain requirement for a split.
```{r}
for (ig in seq(0.005,0.015,0.001)) {
  tree = rpart(adopter ~ ., data = data_train_dt,
               method = "class",
               parms = list(split = "information"),
               control = list(cp = ig))
  pred_tree = predict(tree, data_validation[selected_features_dt], type = "class")
  conf_matrix = confusionMatrix(data = pred_tree,
                                reference = data_validation$adopter,
                                mode = "prec_recall",
                                positive = '1')
...
```
cp (minimum information gain requirement for a split) doesn't change model performance so we use default cp value.
...
```
minsplit (minimum information gain requirement for a split) doesn't change model performance either.
...
```
The performance for current decision tree is 0.1643227, which is equal to or larger than tuning results. Thus, we directly use the decision tree after feature selection with 20% oversampling p for model comparison in Chapter 4.

## 3.5 Naive Bayes Prediction Model
Define a function that trains a Naive Bayes model and returns the performance metric we defined.
```{r}
nb_cv <- function(data_train, data_vali) {
  # build a naive Bayes model
  nb_model = naiveBayes(adopter ~ ., data = data_train)
  # make prediction
  pred_nb = predict(nb_model, data_vali)
  # get the performance
...
The Naive Bayes model with 10% oversampling has the best performance as of 0.139847. We then perform feature selection on it to select the best subset of features to avoid overfitting. We choose backward method because all features have a low correlation coefficient with adopter.
```{r}
# use data with 10% oversampling
...
backward_selection_nb <- function(X,Y,A,B) {
  ...
  for (i in seq_len(n_features)) {
    ...
    for (j in selected_features) {
      ...
      data_vali = cbind(A[current_features],B)
      # get performance
      performance <- nb_cv(data_train,data_vali)
...
```
Based on feature selection results, the Naive Bayes model works best when it only uses subscriber_friend_cnt feature with a performance of 0.1963899. The error occurs at the last round because there should be at least one column as feature.

### 3.5.5 Fine-Tuned Naive Bayes
Train Naive Bayes model only using selected-feature subscriber_friend_cnt with 10% oversampling.
```{r}
# prepare training data
...
# 4. Model Evaluation
Compare 3 different models using the performance metric we defined and conduct cross validation on each of them to test its robustness.

## 4.1 Performance Comparision

Model(fine-tuned) | Performance | Comments
----------------- | ----------- | ---------------------
K-NN              | 0.0966684   | 10% oversampling, K=3, 14 features selected
Decision Tree     | 0.1643227   | 20% oversampling, 4 features selected
Naive Bayes       | 0.1963899   | 10% oversampling, 1 feature selected

## 4.2 Cross Validation
Use the createFolds function to generate 5 groups of row indexes that correspond to each fold.
```{r}
set.seed(123)
data_cv = rbind(data_train, data_validation)
cv = createFolds(y = data_cv$adopter, k = 5)
```

### 4.2.1 K-NN
```{r}
performance_cv_knn = c()
for (validation_rows in cv) {
  data_train_cv <- data_cv[-validation_rows,]
  data_validation_cv <- data_cv[validation_rows,]
  # 10% oversampling
  balanced_data_train_cv <- ovun.sample(adopter ~ ., data = data_train_cv, method <- "over", p = 0.1, seed = 123)$data
  # normalize
  balanced_data_train_norm_cv <- balanced_data_train_cv |> mutate_at(1:25, normalize)
  data_validation_norm_cv = data_validation_cv |> mutate_at(1:25, normalize)
  # feature selection
  balanced_data_train_norm_cv <- balanced_data_train_norm_cv[selected_features_knn]
  data_validation_norm_cv <- data_validation_norm_cv[selected_features_knn]
  # Train model and evaluate its performance
  pred_knn = knn(train = balanced_data_train_norm_cv[,1:13],
             test = data_validation_norm_cv[,1:13],
             cl = balanced_data_train_norm_cv[,14],
             k = 3)
  conf_matrix = confusionMatrix(data = pred_knn, 
                                reference = data_validation_norm_cv$adopter,
                                mode = "prec_recall",
                                positive = '1')
  precision_knn <- conf_matrix$byClass["Pos Pred Value"]
  f1_knn <- conf_matrix$byClass["F1"]
  performance <- 0.7*precision_knn+0.3*f1_knn
  # add the performance of current fold
  performance_cv_knn = c(performance_cv_knn, performance)
}
performance_cv_knn
```
Report the average and the standard deviation for performance
```{r}
cat("K-NN:","avg_performance is", mean(performance_cv_knn),", standard deviation is", sd(performance_cv_knn))
```
...
for (validation_rows in cv) {
  ...
  data_validation_cv <- data_cv[validation_rows,]
  # 20% oversampling
  balanced_data_train_cv <- ovun.sample(adopter ~ ., data = data_train_cv, method <- "over", p = 0.2, seed = 123)$data
  # Train model and evaluate its performance
  tree = rpart(adopter ~ ., data = balanced_data_train_cv[selected_features_dt],
               method = "class",
               parms = list(split = "information"))
  # make prediction
  pred_tree = predict(tree, data_validation_cv[selected_features_dt], type = "class")
  # get the performance
  conf_matrix = confusionMatrix(data = pred_tree,
                              reference = data_validation_cv$adopter,
                              mode = "prec_recall",
                              positive = '1')
  precision_dt <- conf_matrix$byClass["Pos Pred Value"]
  f1_dt <- conf_matrix$byClass["F1"]
  performance <- 0.7*precision_dt+0.3*f1_dt
  # add the performance of current fold
  performance_cv_dt = c(performance_cv_dt, performance)
}
performance_cv_dt
```
Report the average and the standard deviation for performance
```{r}
cat("Decision Tree:","avg_performance is", mean(performance_cv_dt),", standard deviation is", sd(performance_cv_dt))
```
### 4.2.3 Naive Bayes
...
for (validation_rows in cv) {
  ...
  balanced_data_train_cv <- ovun.sample(adopter ~ ., data = data_train_cv, method <- "over", p = 0.1, seed = 123)$data
  # feature selection
  balanced_data_train_cv <- balanced_data_train_cv[c("subscriber_friend_cnt","adopter")]
  data_validation_cv <- data_validation_cv[c("subscriber_friend_cnt","adopter")]
  # Train model and evaluate its performance
  nb_model = naiveBayes(adopter ~ ., data = balanced_data_train_cv)
  pred_nb = predict(nb_model, data_validation_cv)
  conf_matrix = confusionMatrix(data = pred_nb,
                                reference = data_validation_cv$adopter,
                                mode = "prec_recall",
                                positive = '1')
  ...
  precision_nb <- conf_matrix$byClass["Pos Pred Value"]
  f1_nb <- conf_matrix$byClass["F1"]
  performance <- 0.7*precision_nb+0.3*f1_nb
  # add the performance of current fold
  performance_cv_nb = c(performance_cv_nb, performance)
}
performance_cv_nb
```
Report the average and the standard deviation for performance
```{r}
cat("Naive Bayes:","avg_performance is", mean(performance_cv_nb),", standard deviation is ", sd(performance_cv_nb))
```

Cross Validation Results:

Model(fine-tuned) | avg_perform   | sd_perform
----------------- | ------------- | ---------------------
K-NN              | 0.0576954     | 0.0180947
Decision Tree     | 0.1532398     | 0.0151875
Naive Bayes       | 0.1555952     | 0.0196267

The Decision Tree model and Naive Bayes model have better performance than K-NN model. The Decision Tree model has a better robustness than Naive Bayes model and K-NN model.

## 4.3 ROC & AUC
We also make ROC curves and compute AUC metrics for three models.
Get probability of 3 models.
```{r}
# knn
balanced_data_train_norm <- balanced_data_train_10 |> mutate_at(1:25, normalize)
balanced_data_train_norm <- balanced_data_train_norm[selected_features_knn]
data_validation_norm <- data_validation_norm[selected_features_knn]
model_knn <- kknn(adopter ~ ., 
                 train = balanced_data_train_norm, 
                 test =data_validation_norm,
                 k = 3)
knn_probs <- model_knn$prob[,"1"]
# decision tree
dt_probs <- predict(tree_fine_tuned, data_validation, type = "prob")[,"1"]
# naive bayes
nb_probs <- predict(nb_fine_tuned, data_validation, type = "raw")[, "1"]
```
Calculate the ROC curve for each model
```{r}
test_target <- data_validation[, "adopter"]
knn_roc <- roc(test_target == 1, knn_probs)
dt_roc <- roc(test_target == 1, dt_probs)
nb_roc <- roc(test_target == 1, nb_probs)
```
Plot ROC curves for all models in one chart
```{r}
plot(knn_roc, col = "blue", main = "ROC Curves for K-NN, Decision Tree, and Naive Bayes",
     xlab = "False Positive Rate", ylab = "True Positive Rate")
lines(dt_roc, col = "red")
lines(nb_roc, col = "green")
legend("bottomright", legend = c("K-NN", "Decision Tree", "Naive Bayes"),
       col = c("blue", "red", "green"), lwd = 2)
```

Based on ROC curves, Decision Tree model has a better performance than Naive Bayes model.

# 5. Model Application
Considering both performance and robustness, we choose Decision Tree as the best predictive model and use it on testing data to understand how it perform.
...
```{r}
# make prediction
pred_tree = predict(tree_fine_tuned, data_test, type = "class")
# get the performance
conf_matrix = confusionMatrix(data = pred_tree,
...
f1 <- conf_matrix$byClass["F1"]
performance <- 0.7*precision+0.3*f1
cat("The fine-tuned Decision Tree model achives performance as of",performance,"on testing data.")
```

## 5.3 Lift Curve & Lift Ratio
```{r}
predicted_probs = predict(tree_fine_tuned, data_test, type = "prob")
# create lift data
data_test_lift = data_test %>%
  mutate(prob = predicted_probs[,"1"]) %>%
  arrange(desc(prob)) %>%
  mutate(adopter_yes = ifelse(adopter==1,1,0)) %>%
  mutate(x = row_number()/nrow(data_test),
         y = (cumsum(adopter_yes)/sum(adopter_yes))/x)
# get a point to the plot to indicate the desired point (top 20% of data)
desired_percentage <- 0.2
lift_ratio_desired <- data_test_lift$y[which(data_test_lift$x >= desired_percentage)][1]
# plot lift curve
ggplot(data = data_test_lift, aes(x = x, y = y)) +
  ...
  ylab("Lift Ratio") + 
  geom_point(aes(x=desired_percentage,y=lift_ratio_desired),colour="red")
```
```{r}
lift_ratio_desired
```
The Lift Ratio at 20% sample is 2.103217, meaning that the model's top 20% predictions are 2.1x more effective at identifying positive instances compared to a random guessing.
