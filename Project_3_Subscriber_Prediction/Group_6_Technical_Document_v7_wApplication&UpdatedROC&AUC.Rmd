...
```
The performance for current decision tree is 0.1643227, which is equal to or larger than fine-tuning results. Thus, we use the current decision tree with 20% oversampling p for following model comparison.
...
for (validation_rows in cv) {
  ...
  data_validation_cv <- data_cv[validation_rows,]
  # 20% oversampling
  balanced_data_train_cv <- ovun.sample(adopter ~ ., data = data_train_cv, method <- "over", p = 0.2, seed = 122)$data
  # Train model and evaluate its performance
  tree_20 = rpart(adopter ~ ., data = balanced_data_train_cv,
             method = "class",
             parms = list(split = "information"))
  # make prediction
  pred_tree = predict(tree_20, data_validation_cv, type = "class")
  # get the performance
  conf_matrix = confusionMatrix(data = pred_tree,
...
----------------- | ------------- | ---------------------
K-NN              | 0.0568995     | 0.01373743
Decision Tree     | 0.1612595     | 0.01971322
Naive Bayes       | 0.1587425     | 0.03650528
...
Considering both performance and robustness, we select fine-tuned decision tree as our best model.

# 5. Model Application
We choose Decision Tree as the best predictive model and use it on testing data to understand how it perform.
...
```
The Lift Ratio at 20% sample is about 2.10, meaning that the model's top 20% predictions are 2.1x more effective at identifying positive instances compared to a random guessing.
